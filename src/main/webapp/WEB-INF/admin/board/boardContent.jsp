	...
	<table class="table table-bordered">
		...
		<tr>
			<td colspan="4" class="text-right">
				<input type="button" value="돌아가기" onclick="location.href='BoardList.ad?pag=${pag}&pageSize=${pageSize}';" class="btn btn-success"/>
			</td>
		</tr>
	</table>
	<hr/>
		...
		<tr>
			<td>
				<c:if test="${!empty nextVo.title}"> <!-- 다음글이 없으면 나오지 않게 처리 -->
					<a href="BoardContent.ad?idx=${nextVo.idx}">▲ 다음글 : ${nextVo.title}</a><br/>
				</c:if>
				<c:if test="${!empty preVo.title}">  <!-- 이전글이 없으면 나오지 않게 처리 -->
					<a href="BoardContent.ad?idx=${preVo.idx}">▼ 이전글 : ${preVo.title}</a><br/>
				</c:if>
			</td>
		</tr>
	</table>
