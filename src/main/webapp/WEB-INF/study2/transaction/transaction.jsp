...
<div class="container">
	...
	<hr/>
	<pre>
트렌젝션(transaction)이란, 데이터베이스의 상태를 변화시키기 위해서 수행하는 작업의 단위를 뜻한다.
여기서 작업의 단위는 징의어(SQL) 한문장이 아닌, 작업 시작에서 종료(확인)될때 까지의 흐름을 모두 포함한다.
즉, 게시판에 글을 등록한다면 글을 등록하는 insert 와 등록된 글을 보여주는 select 질의어가 정상적으로 종료될때까지의 시점을 말한다.
트렌젝션은 ACID 라는 원자성/일관성/격리성/지속성 을 보장해야한다.
...
■ 트렌젝션의 연산에는 Commit, Rollback 연산이 있다.
...
□ 수행처리는 기본적으로 자동커밋이 설정되어있으며, 트렌젝션을 처리하기 위해서는 수동커밋으로 시작해야한다.
	</pre>
	<hr/>
